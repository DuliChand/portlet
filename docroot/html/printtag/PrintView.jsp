...
<style>
.fancybox-inner .lightboxLogo {
    background: url(http://cdn1.fashionandyou.com/fny-static/app-images/transparent4.png);
    float: left;
    position: absolute;
    top: 0;
    width: 100%;
}
.fancybox-inner .lightboxLogo .centerImg {
    position: relative;
    text-align: center;
    left:36%;
}
...
<ul class="breadcrumb">
	...
	<li class="active">Press Coverage</li>
</ul>
</div>
</div>
...
<ul>
	...
	<li><a href="/print" class="active" id="presstab2" >Press Coverage</a></li>
	<!-- <li><a href="/digital" id="presstab3" >Digital</a></li> -->
<!-- 	<li><a href="/electronic" id="presstab4">Electronic</a></li> -->
	<!-- <li><a href="/awards" id="presstab5">Awards</a></li> -->
	<li><a href="/press-releases" id="presstab1">Press Release</a></li>
</ul>
...
<div class="col-xs-12 col-sm-12 col-md-12" id="press-release-left">
 ...
 <div class="print"><%=journalArticle.getContent()%></div>
 </br>
 <%
  }
  }
 ...
 <%
  List<JournalArticle> journaldigitalArticles =(List<JournalArticle>) renderRequest.getAttribute("journaldigitalArticles");
  if(journaldigitalArticles != null){     
   for(JournalArticle journalArticlesdigital : journaldigitalArticles){
 %>
  <div class="format-new ">
   <%--<span class="press-title-new"><%=journalArticlesdigital.getTitle("en_US")%></span>
    <span class="press-date-new"><%=journalArticle.getDisplayDate()%></span> --%>
  </div> </br>
 <div class="digital"><%=journalArticlesdigital.getContent()%></div>
 </br>
 <%
  }
  }
...
<%
		List<JournalArticle> journalelectronicArticles =(List<JournalArticle>) renderRequest.getAttribute("journalelectronicArticles");
		if(journalelectronicArticles != null){     
			for(JournalArticle journalelectronicsArticle : journalelectronicArticles){
	%>
		<%-- <div class="format-new">
			 <span class="press-title-new"><%=journalelectronicsArticle.getTitle("en_US")%></span> 
			<%-- <span class="press-date-new"><%=journalArticle.getDisplayDate()%></span> 
		</div> </br>
	<div class="electronic"><%=journalelectronicsArticle.getContent()%></div>
	</br>
	<%
		}
		}
...
<script>
	...
	$('.print .press-box .logo a,.print .press-box .content a').on('click',function(event){
 	event.preventDefault();
	$.fancybox($(this).attr('href'));
	$('.fancybox-inner').append('<div class="lightboxLogo"><img class="centerImg" title="FashionAndYou" src="http://cdn1.fashionandyou.com/fny-static/app-images/logo.png" alt="FashionAndYou"></div>' );
	});

/*$(window).load(function(){

setTimeout(function() {
$('.electronic #div3').bxSlider({
	slideWidth:147,
  minSlides: 2,
  maxSlides: 6,
  slideMargin: 9,
